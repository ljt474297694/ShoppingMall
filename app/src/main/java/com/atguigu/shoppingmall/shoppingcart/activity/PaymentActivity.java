...
import com.atguigu.shoppingmall.shoppingcart.adapter.PaymentAdapter;
...
public class PaymentActivity extends AppCompatActivity {
    ...
    @InjectView(R.id.checkOutListView)
    ...
    private void initData() {
        ...
        tvCheckOutAllPrice.setText("总价:" + getTotalPrice() + "￥");
        getTotalPrice();
        if (datas != null && datas.size() > 0) {
            adapter = new PaymentAdapter(this, datas);
            checkOutListView.setAdapter(adapter);
        }
    }

    @OnClick({R.id.tv_left_back, R.id.addressRelative, R.id.btnSureCheckOut})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.tv_left_back:
                finish();
                break;
            case R.id.addressRelative:
                Toast.makeText(PaymentActivity.this, "修改地址", Toast.LENGTH_SHORT).show();
                break;
            case R.id.btnSureCheckOut:
//                Toast.makeText(PaymentActivity.this, "支付", Toast.LENGTH_SHORT).show();
                if (alipay.isChecked()) {
                    pay();
                } else if (weixing.isChecked()) {
                    Toast.makeText(PaymentActivity.this, "微信暂不支持", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(PaymentActivity.this, "请选择一种支付方式", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
    ...
    public void pay() {
        if (TextUtils.isEmpty(PARTNER) || TextUtils.isEmpty(RSA_PRIVATE) || TextUtils.isEmpty(SELLER)) {
            new AlertDialog.Builder(PaymentActivity.this).setTitle("警告").setMessage("需要配置PARTNER | RSA_PRIVATE| SELLER")
...
//                            finish();
        ...
        //生成订单信息-在客户端或者服务器
        String orderInfo = getOrderInfo("硅谷", "测试", getTotalPrice() + "");
